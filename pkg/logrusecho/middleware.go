...
func (lrm *logrusMiddleware) Handle(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		for name, accessor := range lrm.requestData {
			fields[name] = accessor(c.Request())
		}
		...
		logrusWriter := NewLogrusWriter(c.Response().Writer)
		c.Response().Writer = logrusWriter
